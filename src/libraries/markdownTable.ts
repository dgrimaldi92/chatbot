/** biome-ignore-all lint/performance/useTopLevelRegex: TODO to be implemented */
...
function index({ interruptPatterns = [], skipEmptyRows = true } = {}): {
	...
	return {
		...
		// 				"endRegex",
		// 				interruptPatterns.map((str) => `|(?:${str})`).join(""),
		// 			);
...
// 	tableRow: string,
// 	count: number,
// 	prevRow: { colspan: number; rowspan?: number }[] = [],
// 	skipEmptyRows?: boolean,
// ) => {
// 	const cells = [...tableRow.trim().matchAll(/(?:[^|\\]|\\.?)+(?:\|+|$)/g)].map(
// 		(x) => x[0],
// 	);
